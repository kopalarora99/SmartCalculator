...
def hcf(a,b):
    ...
    while H>=1:
        if a%H==0 and b%H==0:
            return H
        H-=1
operation={'ADD':add,'ADDITION':add,'SUM':add, 'PLUS':add,'SUB':sub,'DIFFERENCE':sub,'MINUS':sub,'SUBTRACT':sub,'LCM':lcm,'HCF':hcf,'PRODUCT':mul,'MULTIPLICATION':mul,'MULTIPLY':mul,'DIVISION':div,'DIVIDE':div,'MOD':mod,'REMAINDER':mod,'Modulus':mod}
...
l1.place(x=150,y=10)
l2=Label(win,text='My name is Pug',padx=3)
l2.place(x=180,y=40)
